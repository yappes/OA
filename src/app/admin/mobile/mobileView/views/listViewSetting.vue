          ...
          <div class="pui-content-wrapper__content">
            ...
            <div class="right">
              ...
              <ul class="columns-list">
                <li 
                class="columns-item" 
                v-for="(col, index) in selectedList" 
                :key="col.id"
                draggable="true"
      ...
      <aui-form :model="reqData" :rules="rules" ref="form" label-width="150px">
        ...
        <aui-form-item label="查询条件" prop="searchTag" v-if="viewType == 1" class="search-tag-item">
          ...
          <aui-popover
            ...
            trigger="manual"
            width="620"
            v-model="isConfigSearch"
            popper-class="search-popover"
            @hide="closeConfigSearchDialog">
                ...
                <aui-form-item :key="index" :label="index > 0 ? '' : '查询条件'" :prop="'searchs[' + index + '].columnId'" :rules="{ required: true, validator: validateSourceColumnId, trigger: 'change'}">
                  <div class="condition-list">
                    ...
                    <pui-input-adaptive
                      ...
                      style="width: 30%;"
                      :column="columnMaps[item.columnId]" 
                      :condition-type="item.searchType" 
                      :batch="true" 
                      v-if="columnMaps[item.columnId]"
                      v-model="item.screenVal">
                    </pui-input-adaptive>
            ...
            <pui-btn slot="reference" heightStr="30px" padding="0 10px" style="font-size: 22px" @click="handleOpenConfigSearch">+</pui-btn>
          </aui-popover>
        </aui-form-item>
        ...
        <aui-form-item label="可修改数据关联模块" prop="editRelatedModule" v-if="viewType == 7 || viewType == 6">
        ...
        <aui-form-item label="标题字段" prop="mobileListTitle" v-if="viewType == 1">
          <aui-select v-model="reqData.mobileListTitle" placeholder="请选择">
            <aui-option v-for="col in selectedList" :key="col.id" :value="col.id" :label="col.columnName">
            </aui-option>
          </aui-select>
        </aui-form-item>

        <aui-form-item label="副标题字段" prop="mobileListCaptions" v-if="viewType == 1">
...
export default {
  ...
  methods: {
    ...
    handleSelectAll() {
      ...
      isOrderCol && this.selectedList.forEach(col => {
    ...
    handleViewInfo(res) {
      ...
      res.defaultSearchTagId && (res.defaultSearchTagId = res.defaultSearchTagId.toString());
      !res.viewDataConds &&
        (res.viewDataConds = {
          title: "",
          content: "",
    ...
    initSettingColumns(res) {
      ...
      this.selectedList.forEach(col => {
        ...
        // 可查询字段
        searchColumns.length && searchColumns.forEach(item => {
          item == col.id && this.$set(col, 'isSearch', true);
        })

        // 可模糊搜索字段
        fuzzyQueryCols.length && fuzzyQueryCols.forEach(item => {
    ...
    updateView() {
      ...
      this.selectedList.length && this.selectedList.forEach(col => {
      ...
      // 查询条件
      this.reqData.searchTag.length && (searchTag = JSON.parse(JSON.stringify(this.reqData.searchTag)));
      ...
      params.showColumns = params.mobileShowColumns;
    ...
    findSourceColumns() {
      ...
      viewService.findSourceColumnsByViewId({ 
        viewId: this.editRow.id
      }).then(res => {
    ...
    deleteSearchTag(tagId) {
      viewService.deleteSearchTag({
        ...
        this.reqData.defaultSearchTagId == tagId && (this.reqData.defaultSearchTagId = '');
      });
    },
    ...
    dragenterEvent(index, e) {
      ...
      this.$set(this.selectedList, this.dragIndex, JSON.parse(JSON.stringify(this.selectedList[index])));
