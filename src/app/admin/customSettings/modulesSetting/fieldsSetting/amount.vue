<template>
  <div class="pui-amount">
    <div class="pui-amount__content">
      <aui-form :model="form" :rules="rules" ref="form" label-width="130px" class="form">
        ...
        <aui-form-item label="可输入最小长度" prop="minLen">
          <aui-input-number v-model="form.minLen" controls-position="right" :min="0" :max="99"></aui-input-number>
        </aui-form-item>
        ...
        <aui-form-item v-if='form.columnType==="decimal"' label="小数点后保留位数" prop="decimalLen">
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      //小数点位数正则验证
      var checkDecimalLen = (rule, value, callback) => {
        if (!value) {
          callback();
        } else {
          var myreg = /^[0-9]{1}$/;
          if (!myreg.test(value)) {
            callback(new Error("字段编码为0-9数字1位"));
          } else {
            callback();
          }
        };
      ...
      return {
        ...
        rules: {
          ...
          decimalLen:{validator: checkDecimalLen, trigger: 'blur'},
        },
        columnTypeOpt: [
      ...
      submitForm(formName){
        ...
        form.validate(valid=>{
          if(valid){
            ...
            params.decimalLen = Number(params.decimalLen);
