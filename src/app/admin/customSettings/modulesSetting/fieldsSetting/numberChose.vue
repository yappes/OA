<template>
  <div class="pui-number-chose">
    <div class="pui-number-chose__content">
      <aui-form :model="form" :rules="rules" ref="form" label-width="130px" class="form">
        ...
        <aui-form-item v-if='form.columnType==="decimal"&&false' label="小数点后保留位数" prop="decimalLen">
          <aui-input v-model="form.decimalLen" placeholder="请输入小数点后保留位数"></aui-input>
        </aui-form-item>
        ...
        <aui-form-item label="显示格式" prop="formatType">
          <aui-select v-model="form.formatType" placeholder="请选择显示格式" clearable>
            <aui-option v-for="item in formatTypeOpt" :key="item.id" :label="item.formatTypeName" :value="item.id">
            </aui-option>
          </aui-select>
        </aui-form-item>
  ...
  created() {
    if (this.editRow) {
      ...
      columnService.findFieldById(params).then(data=>{
        ...
        this.form.decimalLen = data.decimalLen
      })
    }
  }
};
