              ...
              @dragover="dragoverEvent">
                ...
                <div class="content" @dragenter.stop="basicDragenterEvent(index, $event)">
                  <aui-form :model="formData" label-width="150px">
                    ...
                    :class="{ 'inline-block-item-wrapper': (col.showType != 5 || col.showType != 10) }"
                  ...
                  <div class="header">
                    ...
                    <div class="title">
                      <aui-form-item label="视图" :prop="'groupLayoutList[' + index + '].batchId'" :rules="{ required: true, validator: validateBatchId, trigger: 'change'}">
                        <aui-select v-model="section.batchId" filterable placeholder="请选择" @change="handleChangeGroupBatch(section.batchId, index)">
                          <aui-option-group
...
export default {
  ...
  data() {
    return {
      rules: {
        ...
        mobileListTitle: [
          { required: true, message: "请选择标题字段", trigger: "change" },
        ],
      },
      isVisible: true,
      loading: true,
  ...
  created() {
    Promise.all([
      ...
      viewService.findBatchViewList(this.viewType === 4 ? { moduleId: this.moduleId, viewTypes: [1], viewFlag: 0 } : { moduleId: this.moduleId, viewTypes: [7], viewFlag: 0 })
    ]).then(resList => {
  ...
  methods: {
    ...
    columnDragendEvent(col, e) {
      ...
      if(!(col.showType == 5 || col.showType == 10)) {
        e.target.className += ' inline-block-item-wrapper';
      }
      this.dragType = '';
    },
