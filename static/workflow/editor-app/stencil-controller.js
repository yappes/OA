...
angular.module('activitiModeler')
  .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
    ...
    $scope.editorFactory.promise.then(function () {
      ...
      console.log("KISBPM", stencilItemGroups);
      ...
      $http({
        ...
        for (var stencilIndex = 0; stencilIndex < data.stencils.length; stencilIndex++) {
          ...
          var stencilItem = {
            ...
            'hide': data.stencils[stencilIndex].hide || false,
        ...
        for (var i = 0; i < stencilItemGroups.length; i++) {
          ...
          var stencilItemGroup = stencilItemGroups[i];
          stencilItemGroup.visible = false;
          for (var j = 0; j < stencilItemGroup.items.length; j++) {
            if (!stencilItemGroup.items[j].hide) {
              stencilItemGroup.visible = true;
              break;
            }
          }
        }
      ...
      $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
        ...
        if (shapes && shapes.length > 0) {
          ...
          $scope.safeApply(function () {
            ...
            $timeout(function () {
              jQuery('div[for="oryx-isforcompensation"]').hide();
            });
          });
    ...
    $scope.setPropertyDefaultValue = function (property) {
      if (property.key === 'oryx-overrideid') {
        if (!Boolean(property.value)) {
          property.value = $scope.guid();
          $scope.updatePropertyInModel(property);
        }
      } else if (property.key === 'oryx-taskdispatch') {
        var selectedItem = $scope.selectedItem;
        var actKey = $scope.workflowModel.key;
        var actId = '';
        var appKey = window.sessionStorage.getItem("paas:cloud:platform:appKey");
        angular.forEach(selectedItem.properties, function (p) {
          if (p.key === 'oryx-overrideid') {
            actId = p.value;
          }
        })
        if ($scope.modelTaskDispatch[actId]) {
          property.value = $scope.modelTaskDispatch[actId];
        } else {
          // 获取远程数据
          axios.get("workflow://event/" + actKey + '/' + actId+"/"+appKey).then(function (res) {
            $scope.$apply(function () {
              property.value = res.data || [];
            });
            $scope.modelTaskDispatch[actId] = res.data;
          })
        }
      }
    }

    $scope.updateTaskDispatch = function (property) {
      // 设置任务调度中心
      var selectedItem = $scope.selectedItem;
      var actId = '';
      angular.forEach(selectedItem.properties, function (p) {
      ...
      $scope.modelTaskDispatch[actId] = property.value;
    }
    ...
    $scope.updatePropertyInModel = function (property, shapeId) {
      ...
      if (newValue != oldValue) {
        if (key === "oryx-taskdispatch") {
          $scope.updateTaskDispatch(property);
          property.mode = 'read';
          return;
        }
