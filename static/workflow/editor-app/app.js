    ...
    $httpProvider.interceptors.push(function($q, $window){
      return {
        'request': function(config){
          ...
          } else if(window.httpVue){
            window.httpVue.confirm()
  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
    ...
    function S4() {
      ...
      $rootScope.guid = function(){
        return (Date.now().toString(36)+"-"+S4()+"-"+S4()+"-"+S4()+"-"+S4()+S4()+S4());
      }
      ...
      function fetchModel(modelId) {
        LoginUser.get().then(function(loginUser){
          console.log("加载当前用户信息：", loginUser);
          $rootScope.loginUser = loginUser;
          return axios.get("workflow://model/" + modelId+"/"+appKey);
        }).then(function(res){
          console.log("加载流程模型信息：", res.data);

          $rootScope.workflowModel = res.data;
      ...
      $rootScope.$on('$includeContentLoaded', function (event) {
        if (!$rootScope.editorInitialized) {
          ...
          if (window['[[initEnvironment]]'] === true) {
            fetchModel(modelId);
          } else {
            window['[[initEnvironmentFn]]'] = function () {
              fetchModel(modelId);
            }
          }
